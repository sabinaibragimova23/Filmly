...
@Component({
  ...
  styles: [`
    ...
    .header-icon { width: 52px; height: 52px; border-radius: 14px; background: linear-gradient(135deg, #e50914, #8b0000); display: flex; align-items: center; justify-content: center; font-size: 24px; flex-shrink: 0; }
    ...
    .podium-label { color: #aaa; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 1rem; }
    .podium { display: flex; gap: 12px; justify-content: center; }
    .podium-card { flex: 1; max-width: 200px; background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 16px; padding: 1.5rem 1rem; display: flex; flex-direction: column; align-items: center; gap: 8px; transition: transform 0.2s; }
    ...
    .podium-avatar { width: 72px; height: 72px; border-radius: 50%; overflow: hidden; background: #333; display: flex; align-items: center; justify-content: center; font-size: 1.6rem; font-weight: 700; color: #fff; }
    ...
    .podium-percent { color: #e50914; font-weight: 700; font-size: 1.2rem; }
    .section-title { color: #888; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 1rem; }
    .actors-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(380px, 1fr)); gap: 12px; }
    .actor-card { background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 12px; padding: 1rem 1.2rem; display: flex; gap: 1rem; align-items: flex-start; transition: border-color 0.2s; }
    .actor-card:hover { border-color: #444; }
    .actor-card.top { border-color: #3a1a1a; }
    .actor-avatar { position: relative; width: 64px; height: 64px; border-radius: 50%; overflow: hidden; flex-shrink: 0; background: #2a2a2a; display: flex; align-items: center; justify-content: center; }
    .actor-avatar img { width: 100%; height: 100%; object-fit: cover; }
    .avatar-fallback { font-size: 1.4rem; font-weight: 700; color: #fff; }
    .top-badge { position: absolute; bottom: -2px; right: -2px; background: #e50914; color: #fff; font-size: 8px; font-weight: 800; padding: 2px 4px; border-radius: 4px; letter-spacing: 0.05em; }
    ...
    .genre-tag { background: #252525; border: 1px solid #333; color: #aaa; font-size: 0.75rem; padding: 2px 8px; border-radius: 20px; }
    .movies-count { color: #666; font-size: 0.78rem; }
    .match-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
    .match-bar-bg { flex: 1; height: 6px; background: #252525; border-radius: 3px; overflow: hidden; }
    ...
    .match-bar-fill.mid  { background: linear-gradient(90deg, #e58c09, #f5a623); }
    ...
    .movie-chip { font-size: 0.72rem; color: #777; background: #1f1f1f; border: 1px solid #2a2a2a; padding: 2px 8px; border-radius: 4px; }
    .skeleton-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(380px, 1fr)); gap: 12px; }
    .skeleton-card { background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 12px; padding: 1.2rem; display: flex; flex-direction: column; gap: 10px; animation: pulse 1.5s ease-in-out infinite; }
    .skeleton-avatar { width: 64px; height: 64px; border-radius: 50%; background: #2a2a2a; }
    ...
    .empty-state { text-align: center; padding: 4rem 2rem; background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 12px; }
    .empty-icon { font-size: 3rem; margin-bottom: 1rem; }
    .empty-state h2 { color: #fff; margin-bottom: 0.5rem; }
    .empty-state p { color: #888; font-size: 0.9rem; }
    .error-banner { background: #3a1a1a; border: 1px solid #e50914; color: #ff6b6b; padding: 1rem 1.5rem; border-radius: 8px; }
    @media (max-width: 600px) { .actors-grid { grid-template-columns: 1fr; } .podium { gap: 8px; } .podium-card { padding: 1rem 0.5rem; } }
  `],
})
export class ActorsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get<ActorRating[]>('http://localhost:8000/api/actors/my-ratings/', {
      ...
      next: data => { this.actors = data; this.loading = false; },
      error: () => { this.error = 'Failed to load actor ratings.'; this.loading = false; }
    });
  }
